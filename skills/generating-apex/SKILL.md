...
description: Primary Apex authoring skill for class generation, refactoring, and review. ALWAYS
  ACTIVATE when the user mentions Apex, .cls, triggers, or asks to create/refactor a class
  (service, selector, domain, batch, queueable, schedulable, invocable, DTO, utility, interface,
  abstract, exception, REST resource). Use this skill for requests involving SObject CRUD,
  mapping collections, fetching related records, scheduled jobs, batch jobs, trigger design,
  @AuraEnabled controllers, @RestResource endpoints, custom REST APIs, or code review of existing
  Apex.
metadata:
  version: 1.0-ibx
  minApiVersion: '66.0'
  upstream: https://github.com/forcedotcom/sf-skills/tree/main/skills/generating-apex
...
compatibility: 'GitHub Copilot in VS Code (Agent mode) or Copilot CLI. Optional MCP: code-review-graph,
  Salesforce DX.'
user-invocable: true
argument-hint: '[class type] [object or goal]'
---
...
> MCP servers live in `.vscode/mcp.json`. Invoke this skill with `/generating-apex` if it does not auto-load.
...
- Class name (derive using the naming table below — apply `PRM_` prefix)
...
6. **Run code analyzer**
   ...
   - Fallback: `sf code-analyzer run --target `. If both are unavailable, record `run_code_analyzer=unavailable: ` in the report.
...
7. **Execute Apex tests**
   ...
   - If unavailable, record `test_execution=unavailable: ` in the report.

### Phase 3 — Report

8. **Report** -- use the output format at the bottom of this file.
   - The `Analyzer` line must contain the actual Step 6 tool output (or `run_code_analyzer=unavailable: ` after attempting invocation).
   - The `Testing` line must contain the actual Step 7 results (or `test_execution=unavailable: ` after attempting invocation).
...
| Avoid `System.debug()` in main code paths | Debug statements evaluate even when logging is not active and consume CPU. Use `PRM_ExceptionLogger` on main code paths |
...
|---|---|---|
| Service | `PRM_{Descriptive}Service` | `PRM_LocationService` |
| Selector | `PRM_{SObject}Selector` | `PRM_AccountSelector` |
| Domain | `PRM_{SObject}Domain` | `PRM_OpportunityDomain` |
| Batch | `PRM_{Descriptive}Batch` | `PRM_AccountDeduplicationBatch` |
| Queueable | `PRM_{Descriptive}Queueable` | `PRM_ExternalSyncQueueable` |
| Schedulable | `PRM_{Descriptive}Schedulable` | `PRM_DailyCleanupSchedulable` |
| DTO | `PRM_{Descriptive}DTO` | `PRM_AccountMergeRequestDTO` |
| Wrapper | `PRM_{Descriptive}Wrapper` | `PRM_OpportunityLineWrapper` |
| Utility | `PRM_{Descriptive}Util` | `PRM_StringUtil` |
| Interface | `PRM_I{Descriptive}` | `PRM_INotificationService` |
| Abstract | `PRM_Abstract{Descriptive}` | `PRM_AbstractIntegrationService` |
| Exception | `PRM_{Descriptive}Exception` | `PRM_LocationServiceException` |
| REST Resource | `PRM_{SObject}RestResource` | `PRM_AccountRestResource` |
| Trigger | `PRM_{SObject}Trigger` | `PRM_AccountTrigger` |
| Trigger Handler | `PRM_{SObject}TriggerHandler` | `PRM_AccountTriggerHandler` |
| Trigger Action | `TA_{SObject}_{Action}` | `TA_Account_SetDefaults` |
| Test | `{ClassName}Test` | `PRM_LocationServiceTest` |

Additional naming rules:
- Classes: `PascalCase`; all custom Apex classes carry the `PRM_` prefix
 ...
 */
public with sharing class PRM_GeolocationService { }
```
 ...
 * @example
 * List<Account> results = PRM_AccountService.deduplicateAccounts(accountIds);
 */
```
...
- Wrap business errors in a custom exception (e.g., `PRM_LocationServiceException`)
...
- `@InvocableVariable` supports: primitives, `Id`, `SObject`, `List<SObject>` only (no `Map`/`Set`/`Blob`); use `List<String>` or `List<SObject>` fields for Flow collection I/O
...
> These rules are IBX-project-specific. They **override or extend** the upstream rules above.
> When there is a conflict, IBX rules win.

### 1 — Naming: `PRM_` Prefix Is Mandatory

Every custom Apex class, trigger, handler, and exception in this project carries the `PRM_` prefix. The upstream generic naming table (`AccountService`, `AccountSelector`, etc.) does **not** apply here.

| Type | IBX Pattern | IBX Example |
|---|---|---|
| Service | `PRM_{Descriptive}Service` | `PRM_AddressManagementService` |
| Selector | `PRM_{SObject}Selector` | `PRM_ProviderSelector` |
| Domain | `PRM_{SObject}Domain` | `PRM_PractitionerDomain` |
| Batch | `PRM_{Descriptive}Batch` | `PRM_RecredConversionBatch` |
| Queueable | `PRM_{Descriptive}Queueable` | `PRM_AddressValidationQueueable` |
| Schedulable | `PRM_{Descriptive}Schedulable` | `PRM_DailyCleanupSchedulable` |
| DTO / Wrapper | `PRM_{Descriptive}DTO` | `PRM_AddressResultDTO` |
| Utility | `PRM_{Descriptive}Util` | `PRM_StringUtil` |
| Exception | `PRM_{Descriptive}Exception` | `PRM_AddressServiceException` |
| Trigger | `PRM_{SObject}Trigger` | `PRM_PractitionerTrigger` |
| Trigger Handler | `PRM_{SObject}TriggerHandler` | `PRM_PractitionerTriggerHandler` |
| Test | `{ClassName}Test` | `PRM_AddressManagementServiceTest` |

### 2 — Trigger Pattern: Bypass Guard Is Mandatory

Every trigger must include the `FeatureManagement.checkPermission` bypass guard before delegating to the handler. No trigger logic is allowed in the trigger body itself.

```apex
trigger PRM_PractitionerTrigger on HealthcareProvider__c (
    before insert, before update, before delete,
    after insert, after update, after delete, after undelete
) {
    if (FeatureManagement.checkPermission('PRM_TriggerBypassPermission') == false) {
        new PRM_PractitionerTriggerHandler().execute();
    }
}
```

- Permission name: `PRM_TriggerBypassPermission` (Custom Permission — do not change)
- The handler class (`PRM_XxxTriggerHandler`) contains all routing logic
- Never add inline business logic to the trigger body

### 3 — Error Logging: Use `PRM_ExceptionLogger` Only

Do **not** create a new logging framework. Do **not** use bare `System.debug()` for exceptions on main code paths. Always call the existing shared logger:

```apex
// Log and continue
PRM_ExceptionLogger.logException(e, 'PRM_MyService', 'methodName');

// Log and return the logged exception record Id (for traceability)
Id logId = PRM_ExceptionLogger.logExceptionReturnId(e, 'PRM_MyService', 'methodName');
```

- First arg: the caught `Exception`
- Second arg: class name (string literal)
- Third arg: method name (string literal)
- Use `logExceptionReturnId` when the caller needs to surface the log ID to the UI or another system

### 4 — Record Type Resolution: Describe-Cache Pattern

Never SOQL against the `RecordType` object. Always resolve by `DeveloperName` via the describe cache:

```apex
// Correct
private static final Map<String, Schema.RecordTypeInfo> RT_INFO_MAP =
    Schema.SObjectType.HealthcareProvider__c.getRecordTypeInfosByDeveloperName();

public static Id getPractitionerRecordTypeId() {
    return RT_INFO_MAP.get('PRM_Practitioner').getRecordTypeId();
}
```

- Use `private static final` so the describe runs once per transaction
- Key by `DeveloperName` (stable across sandboxes), not `Name` (label, can change)

### 5 — Test Coverage Target

- **Minimum:** 75% (Salesforce deploy gate)
- **Target for this project:** ≥ 85%
- Tests must cover: bulk (200-record), single-record, empty-input, and at least one negative/rollback path
- Never use `@isTest(SeeAllData=true)` — create data via test-data factory methods
- Use `Test.startTest()` / `Test.stopTest()` to isolate governor limits and flush async jobs

### 6 — OmniStudio Callable Apex

When an Apex class is called from an Integration Procedure (IP) via the **Apex Remote** action:

```apex
global with sharing class PRM_MyIntegrationService implements omnistudio.VlocityOpenInterface2 {
    ...
    global Boolean invokeMethod(
        ...
        try {
            ...
            PRM_ExceptionLogger.logException(e, 'PRM_MyIntegrationService', methodName);
    ...
    private void doSomething(Map<String, Object> inputMap, Map<String, Object> outMap) {
        // Business logic here; read from inputMap, write results to outMap
    }
}
```

Rules:
- Class must be `global`; method signature is fixed — do not change parameter names or types
- Always implement `omnistudio.VlocityOpenInterface2` (not the older `VlocityOpenInterface`)
- Route by `methodName` string; log unknown methods as errors
- Write all output into `outMap`; return `true` on success, `false` on failure
- Catch all exceptions; log via `PRM_ExceptionLogger`; never let an exception bubble out of `invokeMethod`

### 7 — Quick Review Checklist (IBX)

Before marking any Apex work complete, verify every item:

- [ ] `PRM_` prefix on class name, trigger name, and handler name
- [ ] `with sharing` declared; `without sharing` has a documented justification
- [ ] No SOQL or DML inside any loop
- [ ] All SOQL uses `WITH USER_MODE`; DML uses `AccessLevel.USER_MODE` where applicable
- [ ] No hardcoded IDs — use Custom Metadata, Custom Labels, or describe calls
- [ ] Record types resolved via `getRecordTypeInfosByDeveloperName()`, not SOQL
- [ ] Exceptions logged via `PRM_ExceptionLogger`; no bare `System.debug` for errors
- [ ] Trigger includes `PRM_TriggerBypassPermission` guard
- [ ] If IP-callable: implements `omnistudio.VlocityOpenInterface2`; `invokeMethod` never throws
- [ ] Test class named `{ClassName}Test`; ≥ 85% coverage; bulk + negative paths covered
